...
import { TrendingUp, ShieldAlert, Activity, CheckCircle2, AlertCircle, ArrowRight, CheckCircle, Info } from "lucide-react";
...
export default async function DashboardPage() {
  ...
  return (
    <div className="flex-1 space-y-8 p-8 pt-10 pb-24 max-w-7xl mx-auto w-full">
      ...
      {/* SECTION 1 — PRIMARY FINANCIAL & OPERATIONAL KPIS */}
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        <Card className="shadow-2xs border-slate-200">
          ...
          <CardContent className="p-5 pt-0">
            <div className="text-2xl sm:text-3xl font-bold text-slate-900">{formatINR(metrics.revenueAtRisk, true)}</div>
            <p className="text-xs text-slate-500 mt-1">Unrecovered payment exposure</p>
          </CardContent>
        </Card>

        <Card className="shadow-2xs border-slate-200">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 p-5">
            <CardTitle className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Expected Recovery</CardTitle>
            <TrendingUp className="h-4 w-4 text-indigo-500" />
          </CardHeader>
          <CardContent className="p-5 pt-0">
            <div className="text-2xl sm:text-3xl font-bold text-indigo-600">{formatINR(metrics.expectedRecovery, true)}</div>
            <p className="text-xs text-indigo-600/70 mt-1">Model-estimated recoverable value</p>
          </CardContent>
        </Card>

        <Card className="shadow-2xs border-slate-200 bg-emerald-50/40">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 p-5">
            <CardTitle className="text-xs font-semibold text-emerald-800 uppercase tracking-wider">Recovered Revenue</CardTitle>
            <CheckCircle2 className="h-4 w-4 text-emerald-600" />
          </CardHeader>
          <CardContent className="p-5 pt-0">
            <div className="text-2xl sm:text-3xl font-bold text-emerald-700">{formatINR(metrics.recoveredRevenue, true)}</div>
            <p className="text-xs text-emerald-600 font-medium mt-1">Confirmed captured payments</p>
          </CardContent>
        </Card>

        <Card className="shadow-2xs border-slate-200">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 p-5">
            <CardTitle className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Active Opportunities</CardTitle>
            <Activity className="h-4 w-4 text-slate-400" />
          </CardHeader>
          <CardContent className="p-5 pt-0">
            <div className="text-2xl sm:text-3xl font-bold text-slate-900">{metrics.activeCases}</div>
            <p className="text-xs text-slate-500 mt-1">In recovery lifecycle</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <div className="md:col-span-2 space-y-8">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-600 mt-0.5">
                    {actionRequiredCase.status === 'ready' 
                      ? `${formatINR(actionRequiredCase.amount)} revenue at risk. Recommended action: ${formatActionName(actionRequiredCase.recommendedAction || '')}.`
                      : actionRequiredCase.status === 'recovering'
                      ? `Follow-up in progress for ${formatINR(actionRequiredCase.amount)}. Awaiting customer completion.`
                      : actionRequiredCase.status === 'stopped'
                      ? `Automation stopped by merchant policy guardrails. No further automated retries.`
                      : `Opportunity for ${formatINR(actionRequiredCase.amount)} requires manual intervention.`}
                  </p>
                </div>
              </div>
              <Link href={`/recovery/${actionRequiredCase.id}`} className="shrink-0">
                <Button size="sm" className="w-full md:w-auto font-medium">
                  {actionRequiredCase.status === 'ready' ? 'Review & Execute' : 'View Case'}
          ...
          {/* SECTION 2 — RECOVERY PIPELINE (6 Steps) */}
          <div className="space-y-4">
            <div>
              <h3 className="text-lg font-semibold text-slate-900">Recovery Pipeline</h3>
              <p className="text-sm text-slate-500">Real-time flow of revenue-risk opportunities through automated recovery stages.</p>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-6 gap-2 bg-white border border-slate-200 rounded-lg p-3 shadow-2xs">
              {[
                { label: 'Revenue Event', val: funnel.failedPayments },
                { label: 'Opportunity', val: funnel.opportunities, link: '/recovery' },
                { label: 'Diagnosis', val: funnel.diagnosed },
                { label: 'Decision', val: funnel.policyApproved },
                { label: 'Recovery', val: funnel.executed, link: '/recovery' },
                { label: 'Verified Revenue', val: funnel.recovered, link: '/recovery' }
              ].map((step, i, arr) => (
                <div key={step.label} className="flex flex-col items-center text-center p-2.5 relative group">
                  <p className="text-2xl font-bold text-slate-900">{step.val}</p>
                  <p className="text-[10px] uppercase tracking-wider font-semibold text-slate-500 mt-1.5">{step.label}</p>
                  {step.link && (
                  ...
                  {i < arr.length - 1 && (
                    <div className="hidden md:block absolute right-0 top-1/2 -translate-y-1/2 w-px h-8 bg-slate-200" />
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="space-y-4">
            <div className="flex flex-row items-center justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500">Actionable cases prioritized by expected recoverable value.</p>
              </div>
              <Link href="/recovery">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 w-full items-center">
                        ...
                        <div className="flex flex-col justify-center">
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            <span className="text-xs font-semibold text-slate-700">{Math.round((c.probability || 0) * 100)}%</span>
                          </div>
                        </div>
                        <div className="flex flex-col justify-center">
                          <span className="text-xs text-slate-500">Expected Recovery:</span>
                          <span className="text-sm font-bold text-indigo-600">{formatINR(c.expectedRecovery)}</span>
                        </div>
